...
class CoreRendererServices : public Ri::RendererServices
{
	public:
		...
		{
			return m_renderContext.getApiServices().errorHandler();
		}

		virtual RtFilterFunc getFilterFunc(RtConstToken name) const
		{
			return m_renderContext.getApiServices().getFilterFunc(name);
		}
		virtual RtConstBasis* getBasis(RtConstToken name) const
		{
			return m_renderContext.getApiServices().getBasis(name);
		}
		virtual RtErrorFunc getErrorFunc(RtConstToken name) const
		{
			return m_renderContext.getApiServices().getErrorFunc(name);
		}
		virtual RtProcSubdivFunc getProcSubdivFunc(RtConstToken name) const
		{
			return m_renderContext.getApiServices().getProcSubdivFunc(name);
		}
		...
		{
			// note - we don't want to get the first filter from
			// m_renderContext.getApiServices() because that will give an
			// infinite loop, ricxx2ri -> ri2ricxx -> ricxx2ri
			return m_api;
		}

		virtual void addFilter(const char* name, const Ri::ParamList& filterParams)
		{
			return m_renderContext.getApiServices().addFilter(name, filterParams);
		}

		virtual void parseRib(std::istream& ribStream, const char* name,
							  Ri::Renderer& context)
		{
			return m_renderContext.getApiServices().parseRib(ribStream, name, context);
		}
...
{
	g_context = new CoreContext;
	g_context->renderContext.reset(new CqRenderer());
	g_context->apiServices.reset(new CoreRendererServices(*g_context->renderContext));
...
{
	g_context = static_cast<CoreContext*>(handle);
