...
class RiCxxValidate : public Ri::Renderer
{
    private:
        // Wrapped renderer instance
        Ri::Renderer* m_wrapped;

        Ri::Renderer& wrapped()             { return *m_wrapped; }
        const Ri::Renderer& wrapped() const { return *m_wrapped; }
    ...
    public:
        RiCxxValidate(Ri::Renderer* toWrap)
            : m_wrapped(toWrap),
            m_scopeStack()
        {
            assert(toWrap);
...
$wrapDecl($riCxxMethodDecl($proc, className='RiCxxValidate'), 80)
{
    ...
    m_scopeStack.pop();
#end if
    return wrapped().${procName}(${', '.join($wrapperCallArgList($proc))});
}
'''
...
RtToken RiCxxValidate::Declare(RtConstString name, RtConstString declaration)
{
    ...
    return wrapped().Declare(name, declaration);
}
...
RtVoid RiCxxValidate::FrameBegin(RtInt number)
{
    ...
    return wrapped().FrameBegin(number);
}
...
{
    checkScope(ApiScope(Scope_BeginEnd | Scope_Frame));
    return wrapped().Format(xresolution, yresolution, pixelaspectratio);
}

RtVoid RiCxxValidate::FrameAspectRatio(RtFloat frameratio)
{
    checkScope(ApiScope(Scope_BeginEnd | Scope_Frame));
    return wrapped().FrameAspectRatio(frameratio);
}
...
{
    checkScope(ApiScope(Scope_BeginEnd | Scope_Frame));
    return wrapped().ScreenWindow(left, right, bottom, top);
}
...
{
    checkScope(ApiScope(Scope_BeginEnd | Scope_Frame));
    return wrapped().DepthOfField(fstop, focallength, focaldistance);
}

RtVoid RiCxxValidate::Shutter(RtFloat opentime, RtFloat closetime)
{
    checkScope(ApiScope(Scope_BeginEnd | Scope_Frame));
    return wrapped().Shutter(opentime, closetime);
}

RtVoid RiCxxValidate::PixelVariance(RtFloat variance)
{
    checkScope(ApiScope(Scope_BeginEnd | Scope_Frame));
    return wrapped().PixelVariance(variance);
}

RtVoid RiCxxValidate::PixelSamples(RtFloat xsamples, RtFloat ysamples)
{
    checkScope(ApiScope(Scope_BeginEnd | Scope_Frame));
    return wrapped().PixelSamples(xsamples, ysamples);
}
...
{
    checkScope(ApiScope(Scope_BeginEnd | Scope_Frame));
    return wrapped().Quantize(type, one, min, max, ditheramplitude);
}
...
{
    checkScope(ApiScope(Scope_BeginEnd | Scope_Frame));
    return wrapped().Display(name, type, mode, pList);
}

RtVoid RiCxxValidate::Hider(RtConstToken name, const ParamList& pList)
{
    checkScope(ApiScope(Scope_BeginEnd | Scope_Frame));
    return wrapped().Hider(name, pList);
}
...
{
    checkScope(ApiScope(Scope_BeginEnd | Scope_World | Scope_Object | Scope_Transform | Scope_Attribute | Scope_Solid | Scope_Frame | Scope_Motion));
    return wrapped().TextureCoordinates(s1, t1, s2, t2, s3, t3, s4, t4);
}
                                         ...
                                         const ParamList& pList)
{
    ...
    return wrapped().LightSource(name, pList);
}
...
{
    checkScope(ApiScope(Scope_BeginEnd | Scope_World | Scope_Object | Scope_Transform | Scope_Attribute | Scope_Solid | Scope_Frame | Scope_Motion));
    return wrapped().ShaderLayer(type, name, layername, pList);
}
...
{
    checkScope(ApiScope(Scope_BeginEnd | Scope_World | Scope_Object | Scope_Transform | Scope_Attribute | Scope_Solid | Scope_Frame | Scope_Motion));
    return wrapped().Skew(angle, dx1, dy1, dz1, dx2, dy2, dz2);
}
...
RtVoid RiCxxValidate::CoordinateSystem(RtConstToken space)
{
    ...
    return wrapped().CoordinateSystem(space);
}

RtVoid RiCxxValidate::CoordSysTransform(RtConstToken space)
{
    checkScope(ApiScope(Scope_BeginEnd | Scope_World | Scope_Object | Scope_Transform | Scope_Attribute | Scope_Solid | Scope_Frame | Scope_Motion));
    return wrapped().CoordSysTransform(space);
}
...
{
    checkScope(ApiScope(Scope_Motion | Scope_Object | Scope_Transform | Scope_World | Scope_Attribute | Scope_Solid));
    return wrapped().GeneralPolygon(nverts, pList);
}
...
{
    checkScope(ApiScope(Scope_Motion | Scope_Object | Scope_Transform | Scope_World | Scope_Attribute | Scope_Solid));
    return wrapped().PointsPolygons(nverts, verts, pList);
}
...
{
    checkScope(ApiScope(Scope_Motion | Scope_Object | Scope_Transform | Scope_World | Scope_Attribute | Scope_Solid));
    return wrapped().PointsGeneralPolygons(nloops, nverts, verts, pList);
}
...
{
    checkScope(ApiScope(Scope_BeginEnd | Scope_World | Scope_Object | Scope_Transform | Scope_Attribute | Scope_Solid | Scope_Frame | Scope_Motion));
    return wrapped().Basis(ubasis, ustep, vbasis, vstep);
}

RtVoid RiCxxValidate::Patch(RtConstToken type, const ParamList& pList)
{
    checkScope(ApiScope(Scope_Motion | Scope_Object | Scope_Transform | Scope_World | Scope_Attribute | Scope_Solid));
    return wrapped().Patch(type, pList);
}
...
{
    checkScope(ApiScope(Scope_Motion | Scope_Object | Scope_Transform | Scope_World | Scope_Attribute | Scope_Solid));
    return wrapped().PatchMesh(type, nu, uwrap, nv, vwrap, pList);
}
...
{
    checkScope(ApiScope(Scope_Motion | Scope_Object | Scope_Transform | Scope_World | Scope_Attribute | Scope_Solid));
    return wrapped().NuPatch(nu, uorder, uknot, umin, umax, nv, vorder, vknot, vmin, vmax, pList);
}
...
{
    checkScope(ApiScope(Scope_Motion | Scope_Object | Scope_Transform | Scope_World | Scope_Attribute | Scope_Solid));
    return wrapped().TrimCurve(ncurves, order, knot, min, max, n, u, v, w);
}
...
{
    checkScope(ApiScope(Scope_Motion | Scope_Object | Scope_Transform | Scope_World | Scope_Attribute | Scope_Solid));
    return wrapped().SubdivisionMesh(scheme, nvertices, vertices, tags, nargs, intargs, floatargs, pList);
}
...
{
    checkScope(ApiScope(Scope_Motion | Scope_Object | Scope_Transform | Scope_World | Scope_Attribute | Scope_Solid));
    return wrapped().Sphere(radius, zmin, zmax, thetamax, pList);
}
...
{
    checkScope(ApiScope(Scope_Motion | Scope_Object | Scope_Transform | Scope_World | Scope_Attribute | Scope_Solid));
    return wrapped().Cone(height, radius, thetamax, pList);
}
...
{
    checkScope(ApiScope(Scope_Motion | Scope_Object | Scope_Transform | Scope_World | Scope_Attribute | Scope_Solid));
    return wrapped().Cylinder(radius, zmin, zmax, thetamax, pList);
}
...
{
    checkScope(ApiScope(Scope_Motion | Scope_Object | Scope_Transform | Scope_World | Scope_Attribute | Scope_Solid));
    return wrapped().Hyperboloid(point1, point2, thetamax, pList);
}
...
{
    checkScope(ApiScope(Scope_Motion | Scope_Object | Scope_Transform | Scope_World | Scope_Attribute | Scope_Solid));
    return wrapped().Paraboloid(rmax, zmin, zmax, thetamax, pList);
}
...
{
    checkScope(ApiScope(Scope_Motion | Scope_Object | Scope_Transform | Scope_World | Scope_Attribute | Scope_Solid));
    return wrapped().Disk(height, radius, thetamax, pList);
}
...
{
    checkScope(ApiScope(Scope_Motion | Scope_Object | Scope_Transform | Scope_World | Scope_Attribute | Scope_Solid));
    return wrapped().Torus(majorrad, minorrad, phimin, phimax, thetamax, pList);
}

RtVoid RiCxxValidate::Points(const ParamList& pList)
{
    checkScope(ApiScope(Scope_Motion | Scope_Object | Scope_Transform | Scope_World | Scope_Attribute | Scope_Solid));
    return wrapped().Points(pList);
}
...
{
    checkScope(ApiScope(Scope_Motion | Scope_Object | Scope_Transform | Scope_World | Scope_Attribute | Scope_Solid));
    return wrapped().Curves(type, nvertices, wrap, pList);
}
...
{
    checkScope(ApiScope(Scope_Motion | Scope_Object | Scope_Transform | Scope_World | Scope_Attribute | Scope_Solid));
    return wrapped().Blobby(nleaf, code, flt, str, pList);
}
...
{
    checkScope(ApiScope(Scope_Transform | Scope_Solid | Scope_Attribute | Scope_World | Scope_Object));
    return wrapped().Procedural(data, bound, refineproc, freeproc);
}

RtVoid RiCxxValidate::Geometry(RtConstToken type, const ParamList& pList)
{
    checkScope(ApiScope(Scope_Transform | Scope_Solid | Scope_Attribute | Scope_World | Scope_Object));
    return wrapped().Geometry(type, pList);
}
...
RtVoid RiCxxValidate::SolidBegin(RtConstToken type)
{
    ...
    return wrapped().SolidBegin(type);
}
...
RtVoid RiCxxValidate::ObjectEnd()
{
    ...
    m_scopeStack.pop();
    return wrapped().ObjectEnd();
}

RtVoid RiCxxValidate::ObjectInstance(RtObjectHandle handle)
{
    checkScope(ApiScope(Scope_Transform | Scope_Solid | Scope_Attribute | Scope_World | Scope_Object));
    return wrapped().ObjectInstance(handle);
}
...
RtVoid RiCxxValidate::MotionBegin(const FloatArray& times)
{
    ...
    return wrapped().MotionBegin(times);
}
...
{
    checkScope(ApiScope(Scope_BeginEnd | Scope_Frame));
    return wrapped().MakeTexture(imagefile, texturefile, swrap, twrap, filterfunc, swidth, twidth, pList);
}
...
{
    checkScope(ApiScope(Scope_BeginEnd | Scope_Frame));
    return wrapped().MakeLatLongEnvironment(imagefile, reflfile, filterfunc, swidth, twidth, pList);
}
...
{
    checkScope(ApiScope(Scope_BeginEnd | Scope_Frame));
    return wrapped().MakeCubeFaceEnvironment(px, nx, py, ny, pz, nz, reflfile, fov, filterfunc, swidth, twidth, pList);
}
...
{
    checkScope(ApiScope(Scope_BeginEnd | Scope_Frame));
    return wrapped().MakeShadow(picfile, shadowfile, pList);
}
...
{
    checkScope(ApiScope(Scope_BeginEnd | Scope_Frame));
    return wrapped().MakeOcclusion(picfiles, shadowfile, pList);
}

RtVoid RiCxxValidate::ErrorHandler(RtErrorFunc handler)
{
    checkScope(ApiScope(Scope_BeginEnd | Scope_World | Scope_Object | Scope_Transform | Scope_Attribute | Scope_Solid | Scope_Frame | Scope_Motion));
    return wrapped().ErrorHandler(handler);
}

RtVoid RiCxxValidate::ReadArchive(RtConstToken name, RtArchiveCallback callback,
                                  const ParamList& pList)
{
    return wrapped().ReadArchive(name, callback, pList);
}
//[[[end]]]
...
{
    return boost::shared_ptr<Ri::Renderer>(new RiCxxValidate(toWrap));
}
