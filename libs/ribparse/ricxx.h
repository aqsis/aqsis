...
class Param
{
    ...
    public:
        ...
        Array<RtFloat> getFloatArray() const
        {
            return Array<RtFloat>(static_cast<const RtFloat*>(m_data), m_length);
        }
        Array<RtInt> getIntArray() const
        {
            return Array<RtInt>(static_cast<const RtInt*>(m_data), m_length);
        }
        Array<RtConstToken> getStringArray() const
        {
            return Array<RtConstToken>(static_cast<const RtConstToken*>(m_data), m_length);
        }
};
...
class RendermanInterface
{
    public:
        ...
        virtual RtVoid ColorSamples(const Array<RtFloat>& nRGB,
                            const Array<RtFloat>& RGBn) = 0;
        ...
        virtual RtVoid Color(RtColor Cq) = 0;
        ...
        virtual RtVoid GeneralPolygon(const Array<RtInt>& nverts,
                            const ParamList& pList) = 0;
        virtual RtVoid PointsPolygons(const Array<RtInt>& nverts,
                            const Array<RtInt>& verts,
                            const ParamList& pList) = 0;
        virtual RtVoid PointsGeneralPolygons(const Array<RtInt>& nloops,
                            const Array<RtInt>& nverts,
                            const Array<RtInt>& verts,
                            const ParamList& pList) = 0;
        // ----------------------------- Patches -----------------------------
        ...
        virtual RtVoid PatchMesh(RtConstToken type, RtInt nu,
                            ...
                            const ParamList& pList) = 0;
        virtual RtVoid NuPatch(RtInt nu, RtInt uorder,
                            const Array<RtFloat>& uknot, RtFloat umin,
                            RtFloat umax, RtInt nv, RtInt vorder,
                            const Array<RtFloat>& vknot, RtFloat vmin,
                            RtFloat vmax, const ParamList& pList) = 0;
        virtual RtVoid TrimCurve(const Array<RtInt>& ncurves,
                            const Array<RtInt>& order,
                            const Array<RtFloat>& knot,
                            const Array<RtFloat>& min,
                            const Array<RtFloat>& max, const Array<RtInt>& n,
                            const Array<RtFloat>& u, const Array<RtFloat>& v,
                            const Array<RtFloat>& w) = 0;
        // ----------------------- Subdivision Surfaces ----------------------
        virtual RtVoid SubdivisionMesh(RtConstToken scheme,
                            const Array<RtInt>& nvertices,
                            const Array<RtInt>& vertices,
                            const Array<RtConstToken>& tags,
                            const Array<RtInt>& nargs,
                            const Array<RtInt>& intargs,
                            const Array<RtFloat>& floatargs,
                            const ParamList& pList) = 0;
        // ----------------------------- Quadrics ----------------------------
        ...
        virtual RtVoid Curves(RtConstToken type, const Array<RtInt>& nvertices,
                            RtConstToken wrap, const ParamList& pList) = 0;
        // --------------------- Blobby Implicit Surfaces --------------------
        virtual RtVoid Blobby(RtInt nleaf, const Array<RtInt>& code,
                            const Array<RtFloat>& flt,
                            const Array<RtConstToken>& str,
                            const ParamList& pList) = 0;
        ...
        // ============================== Motion =============================
        virtual RtVoid MotionBegin(const Array<RtFloat>& times) = 0;
        ...
        virtual RtVoid MakeShadow(RtConstString picfile,
                            ...
                            const ParamList& pList) = 0;
        virtual RtVoid MakeOcclusion(const Array<RtConstString>& picfiles,
                            RtConstString shadowfile,
                            const ParamList& pList) = 0;
        // ------------------------------ Errors -----------------------------
