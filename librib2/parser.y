...
void BasisFromName( RtBasis*& b, const char* name);
...
complete_request
	...
	|	pixelfilter string float float
			{
				RendermanInterface::RtFilterFunc pFilter=RiGaussianFilter;
				// TODO: names shoudl be stored elsewhere.
				if(strcmp($2,"box")==0)		pFilter=RiBoxFilter;
				if(strcmp($2,"triangle")==0)	pFilter=RiTriangleFilter;
				if(strcmp($2,"sinc")==0)		pFilter=RiSincFilter;
				if(strcmp($2,"catmull-rom")==0) pFilter=RiCatmullRomFilter;
				if(strcmp($2,"disk")==0)		pFilter=RiDiskFilter;
				if(strcmp($2,"bessel")==0)		pFilter=RiBesselFilter;
	...
	|	option string opttvpairs
			{
				...
				// If this is a 'searchpath' option for archive, then we need this info too.
				if( strcmp($2, "searchpath") == 0 )
				{
					int i;
					for(i=0; i<$3->Count(); i++)
					{
						if( strcmp( $3->Tokens()[i], "archive" ) == 0)
						{
							std::string strPath(*static_cast<char**>($3->Values()[i]));
							UpdateArchivePath(strPath);
						}
					}
				}
    ...
    |   procedural string string scalar_array opttvpairs
            {
                if (!(
		        ...
		        if($4->Count() != 6)
		           yyerror("Expecting 6 Values");
                if (strcmp($2, "RunProgram") == 0)
                   ParseCallbackInterface->RiProcedural($3, &(*$4)[0], (void (*)(void ) ) RiProcRunProgram, NULL);
		        if (strcmp($2, "DelayedReadArchive") == 0)
                   ParseCallbackInterface->RiProcedural($3, &(*$4)[0], (void (*)(void ) ) RiProcDelayedReadArchive, NULL);
		        if (strcmp($2, "DynamicLoad") == 0)
                   ParseCallbackInterface->RiProcedural($3, &(*$4)[0], (void (*)(void ) ) RiProcDynamicLoad, NULL);
                DiscardStringValue($2);
                DiscardStringValue($3);
                DiscardArrayValue($4); 
		        DiscardTokenValuePairs($5);
	...
	|	basis string integer string integer
			{
				...
				RendermanInterface::RtBasis *u = &RiBezierBasis, *v = &RiBezierBasis;
				BasisFromName(u, $2);
				BasisFromName(v, $4);
	...
	|	objectbegin integer
			{
				...
				ParseCallbackInterface->RiObjectBegin();
			}
	|	objectend
	...
	|	objectinstance integer
			{
				...
				ParseCallbackInterface->RiObjectInstance(0);
			}
	...
	|	maketexture	string string string string string float float opttvpairs
			{
				RendermanInterface::RtFilterFunc pFilter=RiGaussianFilter;
				// TODO: names shoudl be stored elsewhere.
				if(strcmp($6,"box")==0)		pFilter=RiBoxFilter;
				if(strcmp($6,"triangle")==0)	pFilter=RiTriangleFilter;
				if(strcmp($6,"sinc")==0)		pFilter=RiSincFilter;
				if(strcmp($6,"catmull-rom")==0) pFilter=RiCatmullRomFilter;
				if(strcmp($6,"disk")==0)		pFilter=RiDiskFilter;
				if(strcmp($6,"bessel")==0)		pFilter=RiBesselFilter;
	...
	|	makelatlongenvironment string string string float float opttvpairs
			{
				RendermanInterface::RtFilterFunc pFilter=RiGaussianFilter;
				// TODO: names shoudl be stored elsewhere.
				if(strcmp($4,"box")==0)		pFilter=RiBoxFilter;
				if(strcmp($4,"triangle")==0)	pFilter=RiTriangleFilter;
				if(strcmp($4,"sinc")==0)		pFilter=RiSincFilter;
				if(strcmp($4,"catmull-rom")==0) pFilter=RiCatmullRomFilter;
				if(strcmp($4,"disk")==0)		pFilter=RiDiskFilter;
				if(strcmp($4,"bessel")==0)		pFilter=RiBesselFilter;
	...
	|	makecubefaceenvironment	string string string string string string string float string float float opttvpairs
			{
				...
				if(strcmp($10,"box")==0)		pFilter=RiBoxFilter;
				if(strcmp($10,"triangle")==0)	pFilter=RiTriangleFilter;
				if(strcmp($10,"sinc")==0)		pFilter=RiSincFilter;
				if(strcmp($10,"catmull-rom")==0) pFilter=RiCatmullRomFilter;
				if(strcmp($10,"disk")==0)		pFilter=RiDiskFilter;
				if(strcmp($10,"bessel")==0)		pFilter=RiBesselFilter;
...
//		{
//			yyerror("Duplicate Light ID Will Be Ignored");
//			return ;
//		}
...
void ParserDeclare(librib::RendermanInterface& CallbackInterface, const std::string Name, const std::string Type)
{
	...
	CallbackInterface.RiDeclare(Name.c_str(), Type.c_str());
}
...
void UpdateArchivePath(std::string strPath)
{
	...
	while ( 1 )
	{
		if ( ( len = strcspn( &strPath[ strt ], "&" ) ) < strlen( &strPath[ strt ] ) )
		{
			str = str + strPath.substr( strt, len );
			str = str + ArchivePath;
			strt += len + 1;
		}
		else
		{
			str = str + strPath.substr( strt );
			break;
		}
	}
	ArchivePath = str;
...
{
	// TODO: names should be stored elsewhere.
	if(strcmp(name,"bezier")==0)			b=&RiBezierBasis;
	else if(strcmp(name,"b-spline")==0)		b=&RiBSplineBasis;
	else if(strcmp(name,"catmull-rom")==0)	b=&RiCatmullRomBasis;
	else if(strcmp(name,"hermite")==0)		b=&RiHermiteBasis;
	else if(strcmp(name,"power")==0)		b=&RiPowerBasis;
}
