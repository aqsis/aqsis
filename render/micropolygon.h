...
public:
    ...
    {
        assert( m_pShaderExecEnv ); return ( m_pShaderExecEnv->vGridRes() );
    }
    virtual	TqInt	GridSize() const
    {
        assert( m_pShaderExecEnv ); return ( m_pShaderExecEnv->GridSize() );
    }
...
struct CqHitTestCache
{
	...
	// these 3 hold values used in doing the edge tests. 1 of each for each edge.
	TqFloat m_YMultiplier[4];
	TqFloat m_XMultiplier[4];
	TqFloat m_Constant[4];
