...
class _qShareC	CqPolygonGeneral2D
{
	...
	_qShareM	std::vector<TqInt>&	aiVertices()			{return(m_aiVertices);}
	_qShareM	TqInt				cVertices() const		{return(m_aiVertices.size());}
	_qShareM	TqInt				Orientation() const		{return(m_Orientation);}
	_qShareM	TqInt				Axis() const			{return(m_Axis);}
	_qShareM	void				SetAxis(TqInt axis)		{m_Axis = axis;}
