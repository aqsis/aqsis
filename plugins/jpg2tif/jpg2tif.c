...
#define	_qshareName jpeg2tif
#include	"share.h"

extern void save_tiff(char *filename, unsigned char *raster, int width, int height, int samples);
...
/*    rgbtriple than save to BMP 24 bitplanes file format.    >*/
/*                                                             */
/*  Returned Value: none                                       */
/***************************************************************/
...
jpeg_open(FILE * infile, char *filename)
{
   ...
   save_tiff(filename, (unsigned char *)buffer, cinfo->output_width , cinfo->output_height, cinfo->num_components);
   free(jerr);
   free(cinfo);
   free(buffer);
   free(jp);
