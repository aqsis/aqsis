...
CqColor CqColor::rgbtohsv() const
{
    ...
    TqFloat max = MAX( m_fRed, m_fGreen );	max = MAX( max, m_fBlue );
    TqFloat min = MIN( m_fRed, m_fGreen );	min = MIN( min, m_fBlue );
    TqFloat diff = max - min;

    V = max;

    // Next Saturation
    if ( max != 0.0 ) S = diff / max;
    else	S = 0.0;

    // Next Hue
    if ( S == 0.0 ) H = UNDEFINED;	// UNDEFINED
    else
    {
        ...
        else if ( G == max ) H = 2.0 + r_dist - b_dist;
        else if ( B == max ) H = 4.0 + g_dist - r_dist;
...
CqColor CqColor::rgbtohsl() const
{
    ...
    TqFloat max = MAX( R, G );	max = MAX( max, B );
    TqFloat min = MIN( R, G );	min = MIN( min, B );
    TqFloat diff = max - min;
