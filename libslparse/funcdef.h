...
class CqFuncDef : public IqFuncDef
{
	...
	virtual	TqBool		fLocal() const	{return(m_fLocal);}
	virtual	const char*	strName() const	{return(m_strName.c_str());}
	virtual	const char*	strVMName() const {return(m_strVMName.c_str());}
	virtual	const char*	strParams() const {return(m_strParamTypes.c_str());}
	...
	virtual	TqInt		InternalUsage() const	{return(m_InternalUses);}
