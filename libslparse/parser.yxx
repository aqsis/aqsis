...
shader_definition
	...
	|	shader_type SYMBOL '(' formals ')' '{' statements '}'
							{
								// Store a pointer to the actual shader.
								CqString strName;
...
unresolvedcall
	:	IDENTIFIER '(' proc_arguments ')'
							{
								pLogger()->warn("Hit unresolved call to function: %s\n", $1->c_str() );
								pLogger()->warn("We will attempt to resolve this with a DSO at runtime\n ");
								CqParseNode* pArgs=$3;
								CqString strArgTypes("");
								if(pArgs)
								{
